  ...
  issue(payload, jwtOptions = {}) {
    ...
    if (mode === 'refresh') {
      ...
      const issueRefreshToken = async () => {
        const refresh = await strapi.sessionManager('users-permissions').generateRefreshToken(userId, undefined, { type: 'refresh' });
  ...
  async verify(token) {
    ...
    if (mode === 'refresh') {
      ...
      const user = await strapi.db.query('plugin::users-permissions.user').findOne({
        where: { id: Number(result.payload.userId) || result.payload.userId },
      });
      if (!user) {
        throw new Error('Invalid token.');
      }
